...
# The figure is created based on the R-code provided by Ariel Ortiz-Bobea
# Web: https://github.com/ArielOrtizBobea/spec_chart
...
# define labels
labels <- list("Weather variables:" = c("precipitation","temperature", "humidity", "still air", "wind components", "radiation"),
               "Functional form:" = c("2nd & 3rd order polynomials", "interactions"))


prep <- function(x){
  
# Add logical values for the included weather variables
k <- c(TRUE, FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,
...
df2           <- as.data.frame(matrix(k,byrow = T, ncol = 8, nrow = 15))
colnames(df2) <- c("precipitation", "temperature", "humidity", "still air", 
                   "wind components", "radiation", "polynomials", "interactions")
df <- cbind(x,df2)

#reorder the specifications
df <- df[c(1:9,10, 12,14, 11, 13, 15),]

# convert to numeric
df <- df %>% mutate_at(.vars = c("V1", "V2"), .funs = ~as.numeric(as.character(.)))

df
}

# function provided by Ariel Ortiz-Bobea
...
# First Stage - three weeks----
load("data/coefficients_specification_chart/first_stage_case_weather.RData")

df <- prep(coefs)

# Create the figure
schart(df, labels = labels, order="asis",
       heights=c(.6,1),ylim = c(0, 0.012),
       pch.dot=c(15,15,15,15),ci=c(0.90,.95),
       cex=c(0.5,0.6),
       highlight=c(7), 
       col.est=c(rgb(0,0,0,0.1),rgb(0.5,0.1,0.59, 0.6)),
       col.est2=c(rgb(0,0,0,0.2),rgb(0.5,0.1,0.55, 0.3)),
       col.dot=c(rgb(0,0,0,0.3),rgb(0,0,0,0.1),rgb(0,0,0,0.1),rgb(0.5,0.1,0.55, 0.7)),
       bg.dot=c(rgb(0,0,0,0.3),rgb(0,0,0,0.1),rgb(0,0,0,0.1),rgb(0.5,0.1,0.55, 0.7)),
       adj=c(0,0), offset=c(6,6),
       leftmargin = 12,
       ylab = "",
       lwd.est = 5.8,
       lwd.symbol = 0.1,
...
load("data/coefficients_specification_chart/first_stage_deaths_weather.RData")

df <- prep(coefs)

# Create the figure
schart(df, labels = labels, order="asis",
       heights=c(.6,1),ylim = c(0, 0.0075),
...
load("data/coefficients_specification_chart/reduced_form_cases_weather.RData")

df <- prep(coefs)

# Create the figure
schart(df, labels = labels, order="asis",
       heights=c(.6,1),ylim = c(0, 0.017),
       pch.dot=c(15,15,15,15),ci=c(0.9,.95),
       cex=c(0.5,0.6),
       highlight=c(7), 
       col.est=c(rgb(0,0,0,0.1),rgb(0.5,0.1,0.59, 0.6)),
...
# Reduced form - COVID-19 deaths----
load("data/coefficients_specification_chart/reduced_form_deaths_weather.RData")

df <- prep(coefs)

# Create the figure
schart(df, labels = labels, order="asis",
       heights=c(.6,1),ylim = c(0, 0.03),
       pch.dot=c(15,15,15,15),ci=c(0.9,.95),
       cex=c(0.5,0.6),
       highlight=c(7), 
       col.est=c(rgb(0,0,0,0.1),rgb(0.5,0.1,0.59, 0.6)),
